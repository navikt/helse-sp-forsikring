...
internal class DataSourceBuilder(env: Map<String, String>) {
    ...
    private val databaseUsername: String = env.envValue("DATABASE_SP_FORSIKRING_OPPRYDDING_DEV_USERNAME")
    private val databasePassword: String = env.envValue("DATABASE_SP_FORSIKRING_OPPRYDDING_DEV_PASSWORD")
    private val databaseName: String = env.envValue("DATABASE_SP_FORSIKRING_OPPRYDDING_DEV_DATABASE")

    private fun Map<String, String>.envValue(key: String) = requireNotNull(get(key)) { "$key must be set" }

    private val hikariConfig = HikariConfig().apply {
        jdbcUrl = "jdbc:postgresql:///%s?%s&%s".format(
            databaseName,
